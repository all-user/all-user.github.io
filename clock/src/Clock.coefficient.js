...
var TimeInfo = require('../../progressbar-with-css-sprite/src/coffee/util/time-info');
...
//}@dev
    ...
    this._frames   = options["frames"] || 60;       // Number   - target FPS.
    ...
    // --- time info ---
    this._timeInfo    = null;                       // TimeInfo - generate time coefficient.
...
Clock["prototype"] = Object.create(Clock, {
    ...
    // --- time info ---
    "timeInfo":     { "get":   function()  { return this._timeInfo; } },
...
function _enterFrame(highResTimeStamp) { // @arg DOMHighResTimeStamp|undefined - requestAnimationFrame give us timeStamp.
    ...
    var info = this._timeInfo.getInfo(timeStamp);

    _callTicks.call(this, info);
}


function _callTicks(info) { // @bind this
...
function Clock_start() {
    if (!this._active) {
        this._active = true;
        this._timeInfo = this._timeInfo || new TimeInfo(this._frames)
